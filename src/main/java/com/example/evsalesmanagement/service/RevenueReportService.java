    ...
    public List<RevenueReportResponseDTO> getRevenueReport(RevenueReportRequestDTO request) {
        ...
        if (request.getStatus() != null && !request.getStatus().isBlank()) {
            try {
                ...
                throw new RuntimeException("Trạng thái không hợp lệ: " + request.getStatus(), e);
            }
        }
    ...
    public byte[] exportRevenueReportToExcel(RevenueReportRequestDTO request) {
        try {
            ...
            try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
                ...
                // Header row
                String[] headers = {"STT", "Loại xe", "Phiên bản", "Màu sắc", "Đại lý", "Số đơn hàng",
                        "Tổng SL", "Tổng doanh thu", "Tổng giảm giá", "Doanh thu thực nhận"};
            ...
            throw new RuntimeException("Lỗi khi xuất báo cáo doanh thu ra Excel", e);
        }
    }
    ...
    private static class RevenueData {
        ...
        RevenueData(OrderDetail detail, Order order) {
            ...
            this.agencyName = Optional.ofNullable(order.getAgency())
                    .map(a -> a.getAgencyName()).orElse("Chưa phân bổ");
        }
